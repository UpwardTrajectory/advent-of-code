...
class Cube(dict):
    ...
    def true_neighbors(self, fullmap: np.array):
        ...
        for vec in self.search_space:
            cur = {"x": self.x + vec[0], "y": self.y + vec[1], "z": self.z + vec[2]}
...
class CubeMap(dict):
    ...
    def read_map(raw_data: str, turns=8):
        ...
        """
        data = [
            [True if x == "#" else False for x in row] for row in raw_data.split("\n")
        ]
        data = [
            [Cube.create(x + turns, y + turns, turns, val) for y, val in enumerate(row)]
        ...
        canvas = np.array(
            [Cube.create(x, y, z, False) for x, y, z in full_grid]
    ...
    def cycle_once(self):
        ...
        two_or_three = np.logical_or(
            self.neighbor_states == 2, self.neighbor_states == 3
        )
