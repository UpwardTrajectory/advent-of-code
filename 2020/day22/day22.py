...
def combat(p1, p2):
    if p1.top > p2.top:
        ...
        raise NotImplementedError(
            "TIE: not sure what to do. Hopefully we never get here."
        )
...
class RecursiveCombat(dict):
    ...
    def build(data):
        ...
        return RecursiveCombat(
            p1=player_1, p2=player_2, deck_histories=set(), meta_lvl="|"
        )
    ...
    def replicate_trimmed_self(self, meta_lvl):
        ...
        meta_out = meta_lvl + "_"
        return RecursiveCombat(
            p1=player_1, p2=player_2, deck_histories=set(), meta_lvl=meta_out
        )
    ...
    def resolve_meta_winner(self, meta_winner, parents):
        ...
        if parents["p1"].name in meta_winner.name:
            ...
            raise ValueError(
                f"MetaWinner: {meta_winner.name} was not cloned from original players {[self.p1.name, self.p2.name]}"
            )
        winner.win(loser.top)
        loser.loss()
        return parents
    ...
    def play_game(self):
        ...
        while p1.cards and p2.cards:
            ...
            if (game_state in self.deck_histories) and not just_exited_recursion:
                ...
                if meta_lvl == "|":
                    print(
                        f"Outermost Game found revisited state on round {i}. {p1.name} wins."
                    )
                return p1
            else:
